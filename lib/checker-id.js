 ...
 */
export async function getCheckerId({
  secretsDir,
  passphrase,
...
async function loadKeypair(keystore, passphrase, { log }) {
  ...
  if (!passphrase) {
    ...
    if (looksLikeJson) {
      const keypair = await tryUpgradePlaintextToCiphertext(
        passphrase,
        keystore,
        ciphertext,
        { log },
 ...
 * @returns
 */
async function tryUpgradePlaintextToCiphertext(
  passphrase,
  keystore,
  maybeCiphertext,
  { log },
...
function parseStoredKeys(json) {
  ...
  assert.strictEqual(
    typeof storedKeys.publicKey,
    'string',
    'checker_id is corrupted: invalid publicKey',
  )
  assert.strictEqual(
    typeof storedKeys.privateKey,
    'string',
    'checker_id is corrupted: invalid privateKey',
  )
  return storedKeys
}
