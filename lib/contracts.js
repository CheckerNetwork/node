...
export const runUpdateContractsLoop = async ({
  provider,
  contracts,
  onActivity,
}) => {
  ...
  while (true) {
    try {
      ...
      console.error(
        'Failed to update the list of contract addresses. Will retry later.',
        err,
      )
      onActivity({
        type: 'error',
  ...
  const contractAddresses = await pRetry(getContractAddresses, {
    ...
    onFailedAttempt: (err) => {
      ...
      if (String(err).includes('You are being rate limited')) {
        ...
        console.error(`Rate limited. Waiting ${delaySeconds} seconds...`)
