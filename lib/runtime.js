...
export const getRuntimeExecutable = ({ runtime, executable }) => {
  return join(runtimeBinaries, runtime, getExecutableFileName(executable))
}
...
export const installRuntime = async ({
  ...
  const target = targets.find(
    (target) => target.platform === os.platform() && target.arch === arch,
  )
